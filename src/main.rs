...
async fn main() -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    let handler = registry.create_handler_from_topic_key(&subscriber_config.config.topic_key)
        .ok_or_else(|| format!("Unknown message type for topic: {}", subscriber_config.config.topic_key))?;
